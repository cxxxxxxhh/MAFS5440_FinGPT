...
PROMPT_END = {
    ...
    "strategy": "\n\nLet's assume your prediction for next week of {symbol} ({start_date} to {end_date}) is {prediction}. " \
...
def get_infos(symbol, data_dir, start_date, end_date, min_past_weeks=1, max_past_weeks=3, with_basics=True):
    ...
    for row_idx, row in df.iterrows():
        ...
        prompt += PROMPT_END['strategy'].format(
            start_date=row['Start Date'],
            end_date=row['End Date'],
            prediction=prediction,
            symbol=symbol
...
def get_prompts_n(symbol_list,date, min_past_weeks,max_past_weeks,  use_basics):
    #result = []
    #symbol_list = random.sample(indices, n_stocks)
    n_weeks = random.randint(min_past_weeks, max_past_weeks)
    # print("---------------------------")
    # print(n_weeks)
    # print("---------------------------")
    df_all = pd.DataFrame()
    for symbol in symbol_list:
        name = "/home/cheam/fintech_project3/FinGPT-master/fingpt/FinGPT_Strategy/data/DOW-30_2023-11-20_2024-11-20/" + symbol + "_2023-11-20_2024-11-20_info.csv"
        df = pd.read_csv(name)
        df_all = pd.concat([df_all, df])
        df_all = df_all.groupby('dates', as_index=False).agg({'prompt': '\n\n'.join})
    # print("---------------------------")
    # print(df_all)
    # print("---------------------------")
    df_all['dates'] = pd.to_datetime(df_all['dates'].apply(lambda x: x[:10]))
    filtered_df = df_all[df_all['dates'] <= date].tail(n_weeks)
    merged_prompt = '\n\n'.join(filtered_df['prompt'])
    return merged_prompt

def get_all_prompts_startegy(index, min_stock_num, max_stock_num, min_past_weeks, max_past_weeks, use_basics):
    all_prompts = []
    m_stocks = random.randint(min_stock_num, max_stock_num+1)
    selected_stocks = random.sample(index, m_stocks)
    print(selected_stocks)
    for date in date_range:
        prompts = get_prompts_n(selected_stocks, date, min_past_weeks, max_past_weeks, use_basics)
        prompts = B_INST + B_SYS + SYSTEM_PROMPT_STRATEGY + E_SYS + prompts + E_INST
        # print(prompts)
        # print('-------------------')
        all_prompts.append(prompts)
    return all_prompts, selected_stocks