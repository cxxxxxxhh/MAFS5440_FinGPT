...
def get_news(symbol, data):
    ...
    for end_date, row in data.iterrows():
        ...
        weekly_news = [
            {
                "date": datetime.fromtimestamp(n['datetime']).strftime('%Y%m%d%H%M%S') if n['datetime']>0 else 'unknown', 
                "headline": n['headline'],
                "summary": n['summary'],
    ...
    for metric, value_list in basic_financials['series']['quarterly'].items():
        for value in value_list:
            ...
            if period and v is not None:
                basic_dict[value['period']].update({metric: v})
            else:
                basic_dict[value['period']].update({metric: 'unknown'})
...
def get_all_infos(symbol_list, data_dir, start_date, end_date, min_past_weeks=1, max_past_weeks=3, with_basics=True):
    
    for symbol in tqdm(symbol_list):
        
        csv_file = f'{data_dir}/{symbol}_{start_date}_{end_date}_info.csv' if with_basics else \
                   f'{data_dir}/{symbol}_{start_date}_{end_date}_nobasics_info.csv'
        ...
        prompts = get_infos(symbol, data_dir, start_date, end_date, min_past_weeks, max_past_weeks, with_basics)
...
def query_to_gpt_strategy(index,data_dir, select_num, min_stock_num, max_stock_num, min_past_weeks, max_past_weeks, with_basics):
    for j in tqdm(range(select_num), desc="Processing"):
        print(j, "开始")
        all_prompts, selected_stocks = get_all_prompts_startegy(index, min_stock_num, max_stock_num, min_past_weeks, max_past_weeks, with_basics)
        csv_file = f'{data_dir}/{selected_stocks}_{start_date}_{end_date}_gpt-4_strategy.csv' if with_basics else \
                   f'{data_dir}/{selected_stocks}_{start_date}_{end_date}_nobasics_gpt-4_strategy.csv'
        ...
        for i, prompt in enumerate(all_prompts):
            ...
            if pd.isna(answer) or answer == "":
                print(f"Answer is NaN or empty for {selected_stocks} - {i}. Stopping the process.")
                sys.exit()  # 或者使用 break 退出循环
            append_to_csv(csv_file, prompt, answer)
        print(j, "结束")
    return all_prompts
...
#         label = label.replace('more than 5', '5+')
        ...
        prompt = re.sub(
            r"Let's assume your prediction for next week \((.*)\) is (up|down) by ((:?.*)%). Provide a summary analysis to support your prediction. The prediction result need to be inferred from your analysis at the end, and thus not appearing as a foundational factor of your analysis.", 
